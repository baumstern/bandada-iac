terraform {
  backend "s3" {
    bucket = "dow-cs2-iac"
    key    = "bandana-prod-terraform-state"
    region = "eu-west-1"
  }
}
...
data "aws_secretsmanager_secret_version" "db_url" {
  secret_id = var.db_url_seceret_arn
}
...
module "api" {
  ...
  secrets = [
    {
      name      = "DB_URL"
      valueFrom = data.aws_secretsmanager_secret_version.db_url.secret_string
    }
  ]
  ...
  container_definitions = {
    ("api") = {
      ...
      environment = [
        ...
        {
          name  = "API_URL"
          value = "${module.alb.dns_name}:3000"
        },
        {
          name  = "DASHBOARD_URL"
          value = "${module.alb.dns_name}:3001"
        },
        {
...
module "client" {
  ...
  load_balancer = {
    service = {
      target_group_arn = module.alb.target_groups["dow-cs2"].arn
...
module "dashboard" {
  ...
  load_balancer = {
    service = {
      target_group_arn = module.alb.target_groups["dashboard"].arn
